import { CreateDepartmentRequest, UpdateDepartmentRequest } from '@yorklog/contracts';
...
export const createDepartment = async (req, res) => {
  const parsed = CreateDepartmentRequest.safeParse(req.body);
...
export const updateDepartment = async (req, res) => {
  const parsed = UpdateDepartmentRequest.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: parsed.error.flatten() });
